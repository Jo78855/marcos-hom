...
export default function OrderAccessPortal() {
  ...
  return <main className="portal-shell" dir="rtl">
    ...
    <section className="portal-grid">
      <article className="portal-card"><h2>بيانات الطلب</h2><dl>
        <div><dt>الخدمة</dt><dd>{order.service_type}</dd></div>
        <div><dt>المقاس</dt><dd>{order.width_m || '-'} × {order.height_m || '-'} متر</dd></div>
        <div><dt>اللون</dt><dd>{order.color || 'حسب الاتفاق'}</dd></div>
        <div><dt>التركيب</dt><dd>{order.installation ? 'شامل التركيب' : 'بدون تركيب'}</dd></div>
        <div><dt>الموعد</dt><dd>{order.installation_date ? new Date(order.installation_date).toLocaleString('ar-KW') : 'يحدد لاحقًا'}</dd></div>
      </dl></article>
      <article className="portal-card"><h2>{isCustomer ? 'بيانات العميل' : 'موقع التنفيذ'}</h2><dl>
        <div><dt>الاسم</dt><dd>{data.customer.name}</dd></div>
        {!isCustomer && <><div><dt>الهاتف</dt><dd>{data.customer.phone || '-'}</dd></div><div><dt>العنوان</dt><dd>{data.customer.address || data.customer.area || '-'}</dd></div></>}
      </dl></article>
      {isCustomer && <article className="portal-card portal-money"><h2>الحساب</h2><dl>
        <div><dt>الإجمالي</dt><dd>{order.total ?? 'حسب الاتفاق'} د.ك</dd></div><div><dt>المدفوع</dt><dd>{order.paid_amount ?? 0} د.ك</dd></div><div><dt>المتبقي</dt><dd>{order.balance ?? 0} د.ك</dd></div>
      </dl></article>}
    </section>
