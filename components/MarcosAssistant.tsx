...
export default function MarcosAssistant({embedded=false}:{embedded?:boolean}){
  ...
  const submitBooking=async(e:FormEvent)=>{
    ...
    const result=await createAssistantOrder({customer_name:customerName,customer_phone:customerPhone,customer_area:area,wall_width:width,wall_height:height,wants_installation:installation,quoted_total:offer?total:null,requested_service:offer?.name_ar||'طلب خاص عبر مساعد ماركوز',customer_note:customerNotes||null,photo_path:photoPath});
