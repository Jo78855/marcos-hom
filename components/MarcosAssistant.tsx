...
export default function MarcosAssistant({embedded=false}:{embedded?:boolean}){
  ...
  const submitBooking=async(e:FormEvent)=>{
    ...
    const fireSizeNote=isFireService(selectedService)&&fireSize?`مقاس جهاز الفاير: ${fireSize} سم`:'';
    const cleanCustomerNotes=customerNotes.replace(/(?:\s*\|\s*)?مقاس جهاز الفاير:\s*\d+\s*سم/g,'').trim();
    const result=await createAssistantOrder({customer_name:customerName,customer_phone:customerPhone,customer_area:area,wall_width:width,wall_height:height,wants_installation:installation,quoted_total:total,requested_service:selectedService,customer_note:[fireSizeNote,cleanCustomerNotes,photoWarning].filter(Boolean).join(' | ')||null,photo_path:photoPath});
