...
export default function MarcosAssistant({embedded=false}:{embedded?:boolean}){
  ...
  const submit=(raw=input)=>{const text=raw.trim();if(!text)return;let reply:string;if(/احجز|حجز|اطلب|طلب/.test(norm(text))){setBookingOpen(true);reply='أكيد، تقدر تسجل طلبك الآن وهيظهر مباشرة في لوحة التحكم. اختار المنتج واكتب الاسم ورقم الهاتف والمنطقة والمقاسات، ثم اضغط «تأكيد وإرسال الطلب».'}else if(/معاينه|معاينة|زيارة|مقاس|صوره|صورة/.test(norm(text))){setBookingOpen(true);reply='تمام. ابعت المقاسات وصورة واضحة للمكان، واكتب الاسم ورقم الهاتف والمنطقة عشان الفريق يراجع التفاصيل ويتواصل معك عند الحاجة.'}else reply=answerFor(text,offers,products);setMessages(v=>[...v,{role:'user',text},{role:'assistant',text:reply}]);setInput('');void speak(reply)};
