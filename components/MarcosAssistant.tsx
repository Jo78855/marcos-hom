...
const wantsBooking=(text:string)=>/احجز|حجز|معاينه|معاينة|زيارة|اطلب|طلب|تأكيد|تاكيد/.test(norm(text));
...
سياسة تسجيل الطلبات: أنت لا تسجل الطلب بنفسك ولا تملك صلاحية تأكيده من المحادثة الصوتية. ممنوع نهائيًا أن تقول للعميل إن الطلب تم تسجيله أو تأكيده إلا بعد أن يملأ نموذج الطلب الظاهر أمامه ويضغط زر تأكيد وتسجيل الطلب وتظهر رسالة نجاح من النظام. إذا طلب العميل الحجز أو التأكيد، اطلب منه استخدام نموذج تسجيل الطلب الظاهر أمامه وإكمال الاسم ورقم الهاتف والمنطقة وعرض الحائط ثم الضغط على زر التأكيد.
...
export default function MarcosAssistant({embedded=false}:{embedded?:boolean}){
  ...
  const openBooking=(notice='أكمل البيانات واضغط «تأكيد وتسجيل الطلب». لن يظهر الطلب في لوحة التحكم قبل نجاح التسجيل.')=>{setBookingOpen(true);setBookingNotice(notice)};
  ...
  const submit=(raw=input)=>{const text=raw.trim();if(!text)return;let reply:string;if(wantsBooking(text)||/مقاس|صوره|صورة/.test(norm(text))){openBooking('أكمل الاسم ورقم الهاتف والمنطقة وعرض الحائط، ثم اضغط «تأكيد وتسجيل الطلب». الصورة والارتفاع اختياريان لكنهما يساعدان في المراجعة.');reply='تمام. فتحت لك نموذج تسجيل الطلب. أكمل البيانات الظاهرة واضغط زر تأكيد وتسجيل الطلب. التسجيل لا يتم قبل الضغط على الزر وظهور رسالة النجاح.'}else reply=answerFor(text,offers,products);setMessages(v=>[...v,{role:'user',text},{role:'assistant',text:reply}]);setInput('');void speak(reply)};
  ...
  const submitBooking=async(e:FormEvent)=>{
    ...
    const{data:created,error}=await supabase.from('orders').insert({customer_name:customerName.trim(),customer_phone:customerPhone.trim(),design_id:null,installation,total,status:photoPath?'measurements_received':'needs_photo',area:area.trim(),wall_width:width,wall_height:height,place_photo_path:photoPath,customer_notes:customerNotes.trim()||null,preferred_contact:'whatsapp',assistant_summary:summary,source:'voice_assistant'}).select('id').single();
    setBookingBusy(false);if(error||!created?.id){if(photoPath)await supabase.storage.from('customer-places').remove([photoPath]);setBookingNotice(`تعذر تسجيل الطلب: ${error?.message||'لم يرجع النظام رقم الطلب'}`);return}
    const orderRef=String(created.id).slice(0,8).toUpperCase();
    const reply=photoPath?`تم تسجيل الطلب بنجاح. رقم المرجع ${orderRef}. المقاسات والصورة وصلوا للفريق للمراجعة.`:`تم تسجيل الطلب بنجاح. رقم المرجع ${orderRef}. المقاسات وصلت، والأفضل تضيف صورة واضحة للحائط لتسهيل المراجعة.`;
