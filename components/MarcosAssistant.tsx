...
const assistantOrderTool={type:'function',name:'create_assistant_order',description:'يسجل طلبًا مؤكدًا في لوحة تحكم ماركوز هوم. لا تستخدمه إلا بعد قراءة ملخص الطلب كاملًا وسماع موافقة صريحة من العميل.',strict:true,parameters:{type:'object',additionalProperties:false,properties:{customer_name:{type:'string',description:'اسم العميل'},customer_phone:{type:'string',description:'رقم هاتف العميل'},customer_area:{type:'string',description:'المنطقة'},wall_width:{type:'number',description:'عرض الحائط؛ يقبل المتر مثل 1.22 أو السنتيمتر مثل 122 وسيحوّله النظام'},wall_height:{type:['number','null'],description:'ارتفاع الحائط؛ يقبل المتر مثل 2.90 أو السنتيمتر مثل 290'},wants_installation:{type:'boolean',description:'هل يريد العميل التركيب'},quoted_total:{type:['number','null'],description:'السعر المؤكد فقط، وإلا null'},requested_service:{type:'string',description:'اسم المنتج أو الخدمة المطلوبة'},customer_note:{type:['string','null'],description:'المكونات واللون والعوائق وأي ملاحظات متفق عليها'}},required:['customer_name','customer_phone','customer_area','wall_width','wall_height','wants_installation','quoted_total','requested_service','customer_note']}};
...
export default function MarcosAssistant({embedded=false}:{embedded?:boolean}){
  ...
  const fillBookingFromAssistant=(args:Partial<AssistantOrderArgs>)=>{
    setBookingOpen(true);
